...
class viewScrapByDate_Working extends StatefulWidget {

  final TestPickerWidget3 fromdt,todt;
  const viewScrapByDate_Working({Key? key,required this.fromdt,required this.todt}) : super(key: key);

  @override
  _viewScrapByDate_WorkingState createState() => _viewScrapByDate_WorkingState();
}

class _viewScrapByDate_WorkingState extends State<viewScrapByDate_Working> {
  ...
  bool demoCompareTwoDates(String scrapdt)
  {
    // todateTxt=todate.selectedDate.toString();
    if(widget.todt.temp.isBefore(widget.fromdt.temp))
      {
        _showMyDialog("Wrong Dates!!", "Please check the from and TO dates given !!!");
        return false;
      }
    else
      {
        var t3=TestPickerWidget3("",0,"");
        List<String> dt=scrapdt.split("/");
        String str=dt[2]+dt[0]+dt[1];print(str);
        t3.temp=DateTime.parse(str);print(str);
        if ((widget.fromdt.temp.isAtSameMomentAs(t3.temp) || widget.fromdt.temp.isBefore(t3.temp)) && (widget.todt.temp.isAfter(t3.temp) || widget.todt.temp.isAtSameMomentAs(t3.temp))){
          print(str+"\n"+t3.temp.toString()+"dateC is between dateA and dateB");
          return true;
        } else {
          print(str+"\n"+t3.temp.toString()+"dateC isn't between dateA and dateC");
          return false;
        }
        ...
        .then((value)
    {
      if(value.docs.isNotEmpty)
      {
        value.docs.forEach((doc)
        {
          ...
          if(demoCompareTwoDates(doc.data()['scrap_date'].toString()))
          {
